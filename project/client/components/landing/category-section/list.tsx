  ...
  return (
    <div className='h-full'>
      ...
      {isLoading && <div className='w-full grid grid-cols-5 gap-4'>
        ...
        {/* map data */}
        {!isLoading && data?.data?.data?.data && data?.data?.data?.length> 0 && <div className='grid grid-cols-5 gap-4'>
          {data?.data?.data?.map((category: TCategory) => <CategoryCard category={category} key={category._id}/>)}
        </div>}

        {/* notfound */}
      {
        !isLoading && data?.data?.data?.data && data?.data?.data?.length === 0
  ...
  return (
    <div className='animate-pulse h-20 w-full bg-gray-200 p-1 rounded'>
      <div className='grid grid-cols-12 gap-2'>
        <div className='h-20 aspect-square bg-gray-300 rounded col-span-4'></div>
